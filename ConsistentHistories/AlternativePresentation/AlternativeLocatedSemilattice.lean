...
instance toLocatedSemilattice (L : AlternativeLocatedSemilattice.{u}) :
    _root_.ConsistentHistories.Foundation.LocatedSemilattices.Basic.LocatedSemilattice L.FlatTime L.Ctrl where
  attest := L.flatAttest
...
def fiberAttestation
  ...
  strongly_contradiction_preserving := by
    ...
    have hraw : _root_.ConsistentHistories.Foundation.LocatedSemilattices.Basic.RawContradicts
      ...
      · have hval : (@LocatedSemilattice.attest Time Ctrl inst) y.1 y'.1 = (@LocatedSemilattice.top Time Ctrl inst) q := congrArg Subtype.val hcontr
        rw [y.2]
        exact hval
    have hout : _root_.ConsistentHistories.Foundation.LocatedSemilattices.Basic.RawContradicts
