import ConsistentHistories.Routes.StrongerSafety.Closure
...
namespace ConsistentHistories.Routes.StrongerSafety.Chains
...
theorem left_chain_rightCompatible_right_inactive_matching_cut
  ...
  rcases
      ConsistentHistories.Routes.PathProperties.InactiveCuts.inactive_implies_containsCut_center
...
theorem left_chain_right_cut_upper_le_start_of_start_right_active
  ...
  exact Nat.le_of_not_gt (by
    ...
    have hinactiveStart :
      ...
      ConsistentHistories.Routes.PathProperties.InactiveCuts.containsCut_brackets_inactive
...
theorem cutPrefixData_pair_upper_inconsistent_of_target_lt
  ...
  have hleftActive : pref.leftDerivation.Active prefUpper := by
    simpa [pref, prefUpper, hleftIdx] using
      ConsistentHistories.Routes.PathProperties.InactiveCuts.cutPrefixData_pre_cut_center_active
        leftData
  have hrightActive : pref.rightDerivation.Active (pref.rightIndex prefUpper) := by
    simpa [pref, prefUpper, hrightIdx, rightCutDeriv] using
      ConsistentHistories.Routes.PathProperties.InactiveCuts.cutPrefixData_final_cut_upper_active
        rightData
  ...
  have hcontrRightLeft :
    ...
    exact
      ConsistentHistories.Routes.PathProperties.Compatibility.contradicts_of_nextIndex_le_and_cutMe_eq
...
theorem cutPrefixData_adjacent_cuts_lower_center_inconsistent
  ...
  have hleftActive :
    ...
    have hactive :=
      (ConsistentHistories.Routes.PathProperties.InactiveCuts.final_cut_endpoints_active
...
end ConsistentHistories.Routes.StrongerSafety.Chains
